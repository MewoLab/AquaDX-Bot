...
COPY apps/qqofbot/package.json /app/apps/qqofbot/
...
COPY packages/adapters/package.json /app/packages/adapters/
...
RUN #cd /app/packages/adapters/ && pnpm run build

COPY apps/qqofbot /app/apps/qqofbot

RUN cd /app/apps/qqofbot && pnpm run build
RUN --mount=type=cache,id=pnpm,target=/pnpm/store,sharing=locked \
    --mount=type=secret,id=npmrc,target=/root/.npmrc \
    pnpm deploy --filter=@clansty/maibot-qofbot --prod deploy
...
COPY apps/qqofbot/docker-entrypoint.sh /app/entrypoint.sh
